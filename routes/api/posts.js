...
router.put('/like/:postId',[authmw],async (req,res)=>{
    try{
        ...
        //const ifLiked=post.likes.includes({user:Object(req.user.id)})
        //const ifLiked=post.likes.includes(Object(req.user.id))
        ...
        //const ifLiked=post.likes.find(like=>like.user===Object(req.user.id)) //undefined
        //const ifLiked=post.likes.find(like=>like.user==Object(req.user.id)) //undefined
        //if(ifLiked){return res.json({ifLiked})}
       ...
       // const ifLiked=post.likes.indexOf(like=>like.user===mongoose.Types.ObjectId(req.user.id)) //-1 
        ...
        //const ifLiked=post.likes.findIndex(like=>like=={user:mongoose.Types.ObjectId(req.user.id)}) 
        //const ifLiked=post.likes.findIndex(like=>like==={user:mongoose.Types.ObjectId(req.user.id)})
        //const ifLiked=post.likes.findIndex(like=>like.user===Object(req.user.id)) //-1
        //const ifLiked=post.likes.findIndex(like=>like.user==Object(req.user.id)) //-1
        //console.log('ifliked test ',ifLiked); if(ifLiked>=0){return res.json({ifLiked})}
        ...
        //const ifLiked=post.likes.filter(like=>like.user==Object(req.user.id)) //[]
        //const ifLiked=post.likes.filter(like=>like.user===Object(req.user.id)) //[]
        //console.log('ifliked',ifLiked); if(ifLiked.length==0){return res.json({ifLiked})}
        ...
        if(ifLiked.length>0){ //if(ifLiked>=0){ //if(ifLiked){
            ...
            console.log('unlike length b4 ',post.likes.length)
            ...
            //const removeIndex=post.likes.map(like=>like.user).indexOf(Object(req.user.id))
            ...
            //await post.save()
            //console.log('unlike length after ',post.likes.length)
            //res.json(post.likes)
            /*
            let likes=post.likes
            likes=post.likes.filter(like=>like.user!=req.user.id) //see ifliked filter variation
            //likes=post.likes.filter(like=>like.user!=Object(req.user.id))//not removed
            //likes=post.likes.filter(like=>like.user!==Object(req.user.id))//not removed
            console.log('unlike ',likes,post.likes) //these two not changed together
            console.log('unlikes.length after ',likes.length,post.likes.length)
            ...
            */
            ...
            //console.log('b4 length',post.likes.length)
            //post.likes=post.likes.filter(like=>like.user!=Object(req.user.id))//not removed
            //post.likes=post.likes.filter(like=>like.user!=Object(req.user.id))//not removed
            //console.log('afterlength ',post.likes.length)
            //await post.save()
            //res.json(post.likes)
            ...
            */
            ...
            //const likesanw=await PostM.findByIdAndUpdate(req.params.postId,{$pull:{likes:{user:Object(req.user.id)}}},{new:true})
            //const likesanw=await PostM.findByIdAndUpdate(mongoose.Types.ObjectId(req.params.postId),{$pull:{likes:{user:Object(req.user.id)}}},{new:true})
            //const likesanw=await PostM.findByIdAndUpdate({_id:mongoose.Types.ObjectId(req.params.postId)},{$pull:{likes:{user:Object(req.user.id)}}},{new:true})
            //see likesanw variations below
            console.log('unlikes length ',likesanw.likes.length)
            res.json(likesanw.likes) //res.json(likesanw)

        }else{ console.log('ifliked',ifLiked) //following methods all work 
            console.log('likes length b4 ',post.likes.length)
            //1
            ...
            //likes.push({user:Object(req.user.id)})//ok
            //likes.unshift({user:Object(req.user.id)})//ok
            ...
            //anw=await PostM.findByIdAndUpdate(Object(req.params.postId),{likes},{new:true})//ok
            //await PostM.findByIdAndUpdate({_id:Object(req.params.postId)},{likes},{new:true})//ok
            ...
            //console.log('likes length after ',post.likes.length,likes.length,anw.likes.length)
            //res.json(anw.likes) //or res.json(anw)
            ...
            //likes.push({user:Object(req.user.id)})
            ...
            //console.log('likes length after',post.likes.length)
            ...
            //.likes.unshift({user:Object(req.user.id)})
            //post.likes.push({user:Object(req.user.id)})
            //await post.save()
            //console.log('likes length after',post.likes.length)
            ...
            //const likesanw=await PostM.findByIdAndUpdate({_id:Object(req.params.postId)},{$addToSet:{likes:{user:Object(req.user.id)}}},{new:true})
            //const likesanw=await PostM.findByIdAndUpdate(Object(req.params.postId),{$push:{likes:{user:Object(req.user.id)}}},{new:true})
            console.log('likesanw length ',likesanw.likes.length)
...
router.delete('/comment/:postId/:commentId',[authmw],async (req,res)=>{
    try{
        ...
        //const commentIndex=post.comments.findIndex(_comment=>_comment._id===mongoose.Types.ObjectId(req.params.commentId))//-1 not found
        //const commentIndex=post.comments.findIndex(_comment=>_comment.id==Object(req.params.commentId))
        //const commentIndex=post.comments.findIndex(_comment=>_comment.id===Object(req.params.commentId))//-1 not found
        //const commentIndex=post.comments.findIndex(_comment=>_comment._id==Object(req.params.commentId)) //-1 not found
        //const commentIndex=post.comments.findIndex(_comment=>_comment._id===Object(req.params.commentId))//-1 not found
    
        console.log('commentIndex index ',commentIndex)
        ...
        //if(post.comments[commentIndex].user===mongoose.Types.ObjectId(req.user.id)) return res.status(401).json({msg:"u're authorised to do so"})//BUT NOT AUTH
        //if(post.comments[commentIndex].user!=Object(req.user.id)) return res.status(401).json({msg:"u're not authorised to do so"})
        //if(post.comments[commentIndex].user!==Object(req.user.id)) return res.status(401).json({msg:"u're not authorised to do so"})
        //if(post.comments[commentIndex].user==Object(req.user.id)) return res.status(401).json({msg:"u're authorised to do so"}) //BUT NOT AUTH
        //if(post.comments[commentIndex].user===Object(req.user.id)) return res.status(401).json({msg:"u're authorised to do so"})//BUT NOT AUTH
        ...
        //const comment=post.comments.find(_comment=>_comment.id==Object(req.params.commentId))
        //const comment=post.comments.find(_comment=>_comment.id===Object(req.params.commentId))//-1 not found
        //const comment=post.comments.find(_comment=>_comment._id==Object(req.params.commentId)) //-1 not found
        //const comment=post.comments.find(_comment=>_comment._id===Object(req.params.commentId))//-1 not found
        ...
        //if(comment.user!=mongoose.Types.ObjectId(req.user.id)) return res.status(401).json({msg:"You're not authorised to do so"})
        //if(comment.user!==Object(req.user.id)) return res.status(401).json({msg:"You're not authorised to do so"})
        //if(comment.user!=Object(req.user.id)) return res.status(401).json({msg:"You're not authorised to do so"})
        ...
        //if(comment.user==mongoose.Types.ObjectId(req.user.id)) return res.status(401).json({msg:"You're authorised to do so"})//but not authorised
        //if(comment.user===Object(req.user.id)) return res.status(401).json({msg:"You're authorised to do so"})//but not authorised
        //if(comment.user==Object(req.user.id)) return res.status(401).json({msg:"You're authorised to do so"})//but not authorised
        ...
        //const removeIndex=post.comments.map(_comment=>_comment.id).indexOf(Object(req.params.commentId))
        //const removeIndex=post.comments.map(_comment=>_comment._id).indexOf(Object(req.params.commentId))//-1
        const removeIndex=post.comments.indexOf(comment)
        console.log('removeIndex ',removeIndex) //should be same as commentIndex
        //console.log(post.comments[0].id,typeof post.comments[0].id, post.comments[0]._id, typeof post.comments[0]._id)//string obj
        ...
        //const comments=post.comments.filter(_comment=>_comment.id!=Object(req.params.commentId))
        //const comments=post.comments.filter(_comment=>_comment.id!==Object(req.params.commentId)) //not removed
        //const comments=post.comments.filter(_comment=>_comment._id!=Object(req.params.commentId)) //not removed
        //const comments=post.comments.filter(_comment=>_comment._id!==Object(req.params.commentId))//not removed
